...
# OVERALL_D_FACTOR = 0.8  # Spanish speaking speed is faster
...
for arpa_vowel in VOWELS_LIST + ['W', 'Y']:
    ...
    CUSTOM_ARPA_SUBS[f'| {arpa_vowel}'] = (f', {arpa_vowel}', '0 =1', '0 =1', '0 =1')

for v1 in VOWELS_LIST:
    for v2 in VOWELS_LIST:
        # Prevent devoicing of P T K between vowels
        CUSTOM_ARPA_SUBS[f'{v1} P {v2}'] = (
            f'{v1} P P {v2}', '=0 0.4 0.4 =2', '=0 0 =1 =2', '=0 0 =1 =2')
        CUSTOM_ARPA_SUBS[f'{v1} T {v2}'] = (
            f'{v1} T T {v2}', '=0 0.4 0.4 =2', '=0 0 =1 =2', '=0 0 =1 =2')
        CUSTOM_ARPA_SUBS[f'{v1} K {v2}'] = (
            f'{v1} K K {v2}', '=0 0.4 0.4 =2', '=0 0 =1 =2', '=0 0 =1 =2')

# Reset final ʎ and θ to L and TH
CUSTOM_ARPA_SUBS['L Y |'] = ('L |', '1 0', '0 0', '0 0')
CUSTOM_ARPA_SUBS['S TH |'] = ('TH', '1', '0', '0')

# Reset initial r to R
CUSTOM_ARPA_SUBS['| R HH R'] = ('| R', '0 1', '0 0', '0 0')
...
#         f', {arpa_consonant} AA2 R', '0 =0 1 1.5', '0 0 0.5 0', '0 =0 0.5 0'
#     )
...
CUSTOM_ARPA_SUBS['EH0 |'] = ('EY0 |', '0.7 =1', '=0 =1', '0 =1')
for arpa_vowel in VOWELS_LIST:
    CUSTOM_ARPA_SUBS[f'EH0 | {arpa_vowel}'] = (f'EY0 , {arpa_vowel}', '0.7 0 =2', '=0 0 =2', '0 0 =2')
...
IPA_REPLACE = {
    'ɟ': 'i',  # mistake in words that start with y + consonant
    'dʒ': 'ʝ',  # dʒ does not occur in Spanish but espeak produces it after n+y (e.g. <en yo>)
    's ˈe r |': 's ˈe ɾ |',  # ser should be sˈeɾ not sˈer (tap, no trill)
    'ɾ ɾ': 'r',  # <rr> should be trilled r
}
...
class SpanishArpaSpeech:
    ...
    def __init__(
        ...
        self.phones_to_arpa, self.phones_to_duration, self.phones_to_pitch, self.phones_to_energy = {}, {}, {}, {}
    ...
    def update_custom_arpa_subs(self, custom_arpa_subs, verbose=False):
        for old_arpa_str, (new_arpa_str, duration_str, pitch_str, energy_str) in custom_arpa_subs.items():
            ...
            self.custom_arpa_subs[old_arpa] = (new_arpa, durations, pitches, energies)
    ...
    def gen_inputs(self, spanish, verbose=False, device=DEVICE):
        if isinstance(spanish, str):
            ipas = phonemize(spanish)
        elif isinstance(spanish, list):
            ipas = spanish
        else:
            raise NotImplementedError('spanish must be string of Spanish text or list of regularized Spanish phones')
    ...
    def gen_audio(
        ...
        if inputs is None:
            arpas, d_factor, p_factor, e_factor = self.gen_inputs(spanish, verbose=verbose, device=device)
        else:
        ...
        with torch.no_grad():
            ...
            if custom_filename:
                ...
                filename = ''.join(spanish) if isinstance(spanish, list) else spanish
                if arpa_in_filename:
